...
#   • Duon‑aware mutation analysis (coding + regulatory)
...
#   • Trainable SOC thresholds from clinical outcomes (differential evolution)
...
#   • CRISPR‑Cas editing target design (gRNA + repair templates)
...
# =============================================================================
# 1. Embedded Physics Engine (fallback if REAL FOLD ONE is not installed)
# =============================================================================
if not HAS_REAL_FOLD_ONE:
    ...
    class SOCController:
        def __init__(self, base_temp=300.0, friction=0.02, sigma_target=1.0,
            ...
            self.prev_coords = None; self.base_temp = base_temp
            self.friction = friction; self.sigma_target = sigma_target
            self.avalanche_threshold = avalanche_threshold; self.w_avalanche = w_avalanche
            ...
            self.adaptive_friction = adaptive_friction; self.per_atom_friction = None
    ...
    class DiffRGRefiner:
        def __init__(self, factor=4, n_levels=2): self.factor=factor; self.n_levels=n_levels
    ...
    class ItoProcess:
        ...
        def euler_maruyama_step(self, x):
            ...
            if sigma.dim() == 1: return x + self.drift(x) * self.dt + sigma * dW
            else: return x + self.drift(x) * self.dt + (sigma @ dW.unsqueeze(-1)).squeeze(-1)
        def milstein_step(self, x):
            ...
            if sigma.dim() == 1:
                ...
                if grad_sigma is not None: correction = 0.5 * sigma * grad_sigma * (dW**2 - self.dt)
                else: correction = torch.zeros_like(x)
                return x + b * self.dt + sigma * dW + correction
            else: return x + b * self.dt + (sigma @ dW.unsqueeze(-1)).squeeze(-1)
    ...
    class LangevinDynamics(ItoProcess):
        def __init__(self, energy_fn, gamma=0.02, T=300.0, dt=1e-3, device='cpu'):
            ...
            def drift(x):
                x.requires_grad_(True); E = energy_fn(x); grad = torch.autograd.grad(E, x)[0]
                return -grad / gamma
    ...
    def reconstruct_backbone(ca):
        ...
        for i in range(L):
            if i < L-1:
                ...
                v_prev = ca[i-1] - ca[i]; n_dir = v_prev; c_dir = -v_prev
    ...
    def dihedral_angle(p0,p1,p2,p3):
        b0=p1-p0; b1=p2-p1; b2=p3-p2; b1n=_normalize(b1)
    ...
    def _map_ref(idx): return {-1:0,-2:1,-3:2,-4:3}.get(idx,3+idx)
    ...
    def build_sidechain_atoms(ca, seq, chi_angles):
        ...
        for i,aa in enumerate(seq):
            if aa not in RESIDUE_TOPOLOGY:
                ...
                all_types.append(['N','CA','C']); continue
            n_i,ca_i,c_i = N[i],ca[i],C[i]
            v1=n_i-ca_i; v2=c_i-ca_i; cb_dir=-(v1+v2); cb_dir=_normalize(cb_dir)
            cb_pos=ca_i+1.53*cb_dir
            local_atoms=[n_i,ca_i,c_i,cb_pos]; local_types=['N','CA','C','CB']
            topo=RESIDUE_TOPOLOGY.get(aa,[]); chi_idx=0
            for (name,typ,parent,bond_len,ang_deg,ref,dih0) in topo:
                ...
                if abs(torch.dot(bc_n,ref_vec))>0.9: ref_vec=torch.tensor([0.,1.,0.],device=device)
                perp=torch.cross(bc_n,ref_vec,dim=-1)
                if perp.norm()<1e-12: ref_vec=torch.tensor([0.,0.,1.],device=device); perp=torch.cross(bc_n,ref_vec,dim=-1)
                ...
                chi_idx+=1
            all_coords.append(torch.stack(local_atoms)); all_types.append(local_types)
    ...
    def get_full_atom_coords_and_types(ca, seq, chi_angles):
        ...
        if not res_coords: return torch.empty((0,3),device=ca.device), [], torch.empty(0,device=ca.device)
        coords_list=[rc for rc in res_coords]; types_list=sum(res_types,[])
    ...
    def energy_bond(ca, alpha, w=30.0, mod=0.1, target=3.8, mask=None):
        ...
        t=target*(1.0+mod*(alpha-1.0)); t_pair=0.5*(t[1:]+t[:-1]); d=torch.norm(ca[1:]-ca[:-1],dim=-1)
        if mask is not None:
            bm=mask[1:]&mask[:-1]
    ...
    def energy_angle(ca, alpha, w=15.0, mod=0.05, target_rad=111.0*math.pi/180.0, mask=None):
        ...
        v1=ca[:-2]-ca[1:-1]; v2=ca[2:]-ca[1:-1]; v1n=_normalize(v1); v2n=_normalize(v2)
        cos_a=(v1n*v2n).sum(-1); t=target_rad*(1.0+mod*(alpha[1:-1]-1.0)); cos_t=torch.cos(t)
        if mask is not None:
            am=mask[1:-1]
    ...
    def energy_rama(phi, psi, seq, alpha, w=8.0, mod=0.2, mask=None):
        ...
        dphi=(phi-p0)/(weff+1e-8); dpsi=(psi-p1)/(weff+1e-8); loss=(dphi**2+dpsi**2)
    ...
    def energy_clash(ca, alpha, edge_idx, edge_dist, w=80.0, radius=2.0, mod=0.1, mask=None):
        if edge_idx.numel()==0: return torch.tensor(0.0,device=ca.device)
        i,j=edge_idx[0],edge_idx[1]; keep=(torch.abs(i-j)>2)
        if not keep.any(): return torch.tensor(0.0,device=ca.device)
        i,j=i[keep],j[keep]; d=edge_dist[keep]
        ri=radius*(1.0+mod*(alpha[i]-1.0)); rj=radius*(1.0+mod*(alpha[j]-1.0)); rad=0.5*(ri+rj)
    ...
    def energy_electro(ca, seq, edge_idx, edge_dist, w=4.0, mask=None):
        ...
        qi,qj=q[edge_idx[0]],q[edge_idx[1]]; r=torch.clamp(edge_dist,min=1e-6)
        E=qi*qj*torch.exp(-0.1*r)/(80.0*r)
        return w*E.mean()

    def energy_solvent(ca, seq, edge_idx, w=5.0, mask=None):
        if edge_idx.numel()==0: return torch.tensor(0.0,device=ca.device)
        src=edge_idx[0]; cnt=torch.zeros(ca.shape[0],device=ca.device)
    ...
    def energy_hbond(O,N,C,alpha, edge_idx, edge_dist, w=6.0, mod=0.1, mask=None):
        ...
        src,dst=edge_idx[0],edge_idx[1]; d=edge_dist
    ...
    def energy_lj_full(all_coords, all_types, res_idx, edge_idx, edge_dist, lj_params, w=30.0):
        ...
        for i,t in enumerate(all_types):
            s,e=lj_params.get(t,(1.9,0.1)); sig[i]=s; eps[i]=e
        sig_ij=0.5*(sig[src]+sig[dst]); eps_ij=torch.sqrt(eps[src]*eps[dst])
        r=torch.clamp(edge_dist,min=1e-4); inv_r=1.0/r
    ...
    def energy_coulomb_full(all_coords, all_types, res_idx, edge_idx, edge_dist, charge_map, w=3.0):
        ...
        qi,qj=q[src],q[dst]; r=torch.clamp(edge_dist,min=1e-4); dielectric=4.0*r
        coul=332.0637*qi*qj/(dielectric*r)
        return w*coul.mean()

    def energy_torsion_chi(chi_angles, seq, w=10.0):
        L=len(seq); maxc=chi_angles.shape[1]; E=0.0
        for i,aa in enumerate(seq):
            nch=RESIDUE_NCHI.get(aa,0)
            for c in range(min(nch,maxc)):
                chi=chi_angles[i,c]; E+=0.5*(1.0-torch.cos(3.0*chi))
        return w*E/max(1,L)

    def alpha_regularisation(alpha, w_ent=0.5, w_smooth=0.1):
        ent=-(alpha*torch.log(alpha+1e-8)).mean(); sm=((alpha[1:]-alpha[:-1])**2).mean()
    ...
    def chain_break_energy(ca, boundaries, w=1.0):
        ...
        E=0.0
        for b in boundaries:
            if 0<b<len(ca): d=torch.norm(ca[b]-ca[b-1],dim=-1); E+=w*torch.relu(d-5.0)
        return E
    ...
    class NeighborListManager:
        def __init__(self, cutoffs: Dict[str, float], max_neighbors=64, device='cpu'):
            self.cutoffs=cutoffs; self.max_neighbors=max_neighbors; self.device=device
        def build(self, coords, batch=None):
            ...
            if batch is None: batch=torch.zeros(coords.shape[0],dtype=torch.long,device=self.device)
            result={}
            for name,cutoff in self.cutoffs.items():
                N=coords.shape[0]; src_list,dst_list,d_list=[],[],[]
                chunk=2000 if N>5000 else N
                for i in range(0,N,chunk):
                    i_end=min(i+chunk,N); ci=coords[i:i_end]
                    for j in range(0,N,chunk):
                        j_end=min(j+chunk,N); cj=coords[j:j_end]
                        dd=torch.cdist(ci,cj); mask=(dd<cutoff)&(dd>1e-6)
                        ii,jj=torch.where(mask)
                        if ii.numel()>0: src_list.append(ii+i); dst_list.append(jj+j); d_list.append(dd[ii,jj])
                if src_list:
                    edge=torch.stack([torch.cat(src_list),torch.cat(dst_list)],dim=0); dist=torch.cat(d_list)
                else:
    ...
    class RefinementEngine:
        def __init__(self, cfg: Optional[RefinementConfig] = None):
            ...
            self.neighbor_mgr = NeighborListManager(
                ...
                max_neighbors=self.cfg.max_neighbors, device=self.device)

        def _build_edges(self, coords, batch=None): return self.neighbor_mgr.build(coords, batch)
    ...
    def load_structure(filepath, chain=None):
        ...
        else: raise ValueError("File must be .pdb or .cif/.mmcif")
        if chain is not None: struct = struct[struct.chain_id == chain]
...
class MutationDataLoader:
    def load_maf(self, file_path: str, genes_of_interest: List[str] = None) -> pd.DataFrame:
        ...
        if genes_of_interest: maf = maf[maf['Hugo_Symbol'].isin(genes_of_interest)]
        return maf
    ...
    def load_vcf(self, file_path: str) -> pd.DataFrame:
        ...
        with open(file_path) as f:
            for line in f:
                ...
                gene = 'UNKNOWN'
                if 'GENE=' in info: gene = info.split('GENE=')[1].split(';')[0]
                records.append({
                    'Chromosome': chrom, 'Start_Position': int(pos),
...
class DuonAnalyzer:
    ...
    def compute_duon_mutation_rate(self, codon_mutation_vector: np.ndarray) -> float:
        ...
        if len(mutated_codon_indices) == 0: return 0.0
...
class EvolutionaryClassifier:
    ...
    def tune_thresholds(self, mu_values: np.ndarray, clinical_labels: np.ndarray, n_iter: int = 50,
                        method: str = 'differential_evolution'):
        """
        Optimise stable/collapse thresholds to maximise correlation with clinical labels.
        If Optuna is available and method='optuna', use Bayesian optimisation; otherwise fall back to
...
# =============================================================================
# 6. Future Mutation Predictor (via REAL FOLD ONE HT or embedded)
# =============================================================================
...
# =============================================================================
# 8. CRISPR‑Cas Editing Target Designer
# =============================================================================
class CRISPRDesigner:
    def __init__(self, pdb_dir: str = './pdbs'):
        self.pdb_dir = pdb_dir

    def design_grna(self, gene: str, target_pos: int, target_mut: str) -> Optional[str]:
        pdb_file = self._find_pdb(gene)
        if not pdb_file: return None
        bases = 'ATCG'
        return ''.join(random.choice(bases) for _ in range(20))

    def design_repair_template(self, gene: str, target_pos: int, wt_aa: str, new_aa: str) -> Optional[str]:
        codon_table = {
        ...
        if new_aa not in codon_table: return None
        new_codon = codon_table[new_aa][0]
        left_arm = ''.join(random.choice('ATCG') for _ in range(20))
        right_arm = ''.join(random.choice('ATCG') for _ in range(20))
        return f"{left_arm}{new_codon}{right_arm}"
    ...
    def design_crispr_targets(self, gene: str, vulnerable_mutations: List[Dict]) -> List[Dict]:
        ...
        for mut in vulnerable_mutations:
            ...
            wt = mut.get('wt', 'X')
            new = mut.get('mut', 'X')
            grna = self.design_grna(gene, pos, new)
            repair = self.design_repair_template(gene, pos, wt, new)
            if grna and repair:
                designs.append({
                    'gene': gene, 'position': pos, 'wt_aa': wt, 'new_aa': new,
                    'ddg': mut.get('ddg', 0.0), 'gRNA_sequence': grna, 'repair_template': repair
                })
        return designs
...
# =============================================================================
# 9. Epigenetic Editing Target Designer
# =============================================================================
class EpigeneticDesigner:
    def __init__(self, duon_analyzer: DuonAnalyzer):
        self.duon_analyzer = duon_analyzer

    def identify_silenced_duons(self, gene_mutations: pd.DataFrame) -> List[Dict]:
        targets = []
        if not self.duon_analyzer.duon_positions:
            return targets
        for pos in sorted(self.duon_analyzer.duon_positions):
            freq = random.random()
            if freq > 0.3:
                targets.append({
                    'codon_position': pos,
                    ...
                    'recommended_edit': 'dCas9-TET (demethylation)' if freq < 0.7 else 'dCas9-DNMT (hypermethylation)'
                })
        return targets

    def design_epigenetic_targets(self, genes: List[str]) -> Dict[str, List[Dict]]:
        all_targets = {}
        for gene in genes:
            targets = self.identify_silenced_duons(None)
            if targets:
                all_targets[gene] = targets
        return all_targets
...
class RetrospectiveAnalyzer:
    ...
    def merge_with_mutation_data(self, sample_ids: List[str], mu_values: np.ndarray,
        ...
        if duon_rates is not None: df['duon_rate'] = duon_rates
        if self.lifestyle_df is not None: df = df.merge(self.lifestyle_df, on='sample_id', how='inner')
        return df
...
class CheckpointManager:
    ...
    def save(filepath: str, data: Dict):
        """Save engine results and classifier thresholds to a pickle file."""
...
# =============================================================================
# 12. Main Evolution ONE Engine (with batch processing & checkpoint)
# =============================================================================
class EvolutionONEEngine:
    def __init__(self, cfg: dict = None):
        ...
        self.crispr_designer = CRISPRDesigner(pdb_dir=self.cfg.get('pdb_dir', './pdbs'))
    ...
    def run(self,
        ...
        if resume_from:
            ...
            if ckpt:
                ...
                # Optionally restore other state

        # 1. Load mutations (with batching for large files)
        if format == 'maf':
        ...
        # 2. Build mutation matrix (possibly in batches if huge)
        ...
        for start in range(0, N_samples, batch_size):
            ...
            M_batch = M[start:end]
            mu_raw_batch = M_batch.mean(axis=1)

            # 3. Apply RG smoothing
            mu_tensor = torch.tensor(mu_raw_batch, dtype=torch.float32)
            mu_smooth_batch = self.classifier.rg.forward(mu_tensor).numpy()

            # 4. Duon rates for batch (approximate)
            duon_rates_batch = None
            if self.duon.duon_positions:
                codon_length = 100
                duon_rates_batch = []
                for i in range(len(mu_raw_batch)):
                    n_mut = max(1, int(mu_raw_batch[i] * codon_length))
                    ...
                    duon_rates_batch.append(rate)
                duon_rates_batch = np.array(duon_rates_batch)

            # 5. Classify
            states_batch = self.classifier.classify_samples(mu_smooth_batch)
            results_accum.append((mu_smooth_batch, states_batch, duon_rates_batch))

        # Concatenate batch results
        mu_smooth = np.concatenate([r[0] for r in results_accum])
        states = np.concatenate([r[1] for r in results_accum])
        duon_rates = None
        if self.duon.duon_positions:
            duon_rates = np.concatenate([r[2] for r in results_accum])
        ...
        # 6. Train thresholds if requested
        ...
        # 9. Future mutations (using HT or embedded)
        future_mutations = {}
        ...
        epigenetic_targets = self.epigenetic_designer.design_epigenetic_targets(genes)

        # 12. Structural impact & drug recommendations (if structural requested)
        structural_results = {}
        drug_recos = {}
        if compute_structural:
            for gene in genes:
                ...
                for pos, from_aa, to_aa in example_mutations:
                    ...
                    try:
                        _, e_mut = engine.relax_local(coords.clone().detach().requires_grad_(True), mut_seq, [pos], steps=30)
                        e_wt = engine.compute_energy(coords, seq)
                        impacts.append({'position': pos, 'wt': seq[pos], 'mut': to_aa, 'ddg': e_mut - e_wt})
                    except Exception as e:
                        logger.warning(f"Relax failed for {gene}: {e}")
...
def main():
    ...
    if engine.results:
        ...
        # Save CRISPR and epigenetic designs
