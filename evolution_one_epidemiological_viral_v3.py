...
class LearnableRG(nn.Module):
    ...
    DIFF-FIX 1: kernel weights normalised via softmax (not clamp/sum)
    so gradients flow through the normalisation.
    """
    def __init__(self, kernel_size: int = 5):
        super().__init__()
        self.raw_weight = nn.Parameter(torch.ones(kernel_size))
        self.padding    = kernel_size // 2

    @property
    def weight(self) -> torch.Tensor:
        """Softmax-normalised kernel — always positive, sums to 1."""
        return F.softmax(self.raw_weight, dim=0)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.dim() == 1:
            x = x.unsqueeze(0)
        w = self.weight.view(1, 1, -1)
        return F.conv1d(x.unsqueeze(0), w, padding=self.padding).squeeze(0)


class DifferentiableSOC(nn.Module):
    """
    Differentiable Self-Organised Criticality dynamics.

    DIFF-FIX 2: std() replaced with smooth logsumexp-based spread
    estimator; scale uses softplus to ensure positivity of T.
    """
    def __init__(self, base_temp: float = 300.0, beta: float = 0.01):
        super().__init__()
        self.log_base_temp = nn.Parameter(torch.tensor(math.log(base_temp)))
        self.log_beta      = nn.Parameter(torch.tensor(math.log(beta)))

    @property
    def base_temp(self) -> torch.Tensor:
        return torch.exp(self.log_base_temp)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # DIFF-FIX 2: soft spread instead of std()
        tau   = 0.1
        xf    = x.reshape(-1)
        spread = tau * torch.logsumexp(xf / tau, dim=0) \
                 - tau * torch.logsumexp(-xf / tau, dim=0)
        T     = _softplus_floor(
                    self.base_temp * (1.0 + self.beta * (spread - 1.0)),
                    floor=1.0)
...
class LangevinEvolutionBridge:
    """
    Bridge: Langevin MD structural stress → epidemic Rt modulation.

    When an AdvancedStructuralLangevin integrator (from structural_langevin)
    is available, this bridge injects the SSC-filtered structural stress
    into the EpidemicClassifier as an Itô drift correction, capturing the
    idea that molecular-scale conformational dynamics influence viral
    transmissibility and hence Rt.

    Usage::

        bridge = LangevinEvolutionBridge(integrator, classifier)
        bridge.sync(coords, velocities)   # each MD step
        logits = classifier(rt_values)    # Rt now modulated by MD stress
    """

    def __init__(self, langevin, classifier: EpidemicClassifier,
                 scale: float = 0.01):
        self.lang       = langevin
        self.clf        = classifier
        self.scale      = scale

    def sync(self, coords: torch.Tensor, velocities: torch.Tensor) -> None:
        """Project Langevin SSC stress onto classifier Itô drift."""
        with torch.no_grad():
            sigma = self.lang.thermostat.ssc.prev_sigma
        # Differentiable soft update of ito_drift
        self.clf.ito_drift.data = (
            self.clf.ito_drift.data * 0.9
            + self.scale * sigma.detach() * 0.1
        )
