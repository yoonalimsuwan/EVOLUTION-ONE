...
class EvolutionaryClassifier:
    ...
    def classify_samples_differentiable(
        ...
        """
        p_stable   = 1.0 - torch.sigmoid((mu_tensor - self.threshold_stable)  * 20.0)
        p_collapse = torch.sigmoid((mu_tensor - self.threshold_collapse) * 20.0)
        p_critical = torch.clamp(1.0 - p_stable - p_collapse, min=0.0)
        return torch.stack([p_stable, p_critical, p_collapse], dim=-1)
