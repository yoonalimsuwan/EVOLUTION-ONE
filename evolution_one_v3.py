...
from one_core_evolution import (
    ...
    EpiEvolutionBridge,          # Bug 7: EvolutionONE ↔ EpiForecast bridge
...
class EvolutionaryClassifier:
    ...
    def __init__(self, threshold_stable=0.2, threshold_collapse=0.8):
        ...
        # Fully differentiable modules from one_core_evolution
    ...
    def classify_samples_differentiable(
        ...
        """
        Fully differentiable soft classification via sigmoid.
        Returns (N, 3) probability tensor: [p_stable, p_critical, p_collapse].
        Gradients flow through mu_tensor for end-to-end training.
        """
        # Bug 5 fix: log-softmax over three logits (replaces clamp on residual)
        # Three logits: stable (low μ), critical (mid μ), collapse (high μ)
    ...
    def soc_evolve(self, mu_values: torch.Tensor, steps: int = 10) -> torch.Tensor:
        ...
        Replaces the prototype loop that used .detach() + manual clamping
        and broke the gradient graph.
        """
    ...
    def ito_evolve(self, mu0: float, T: float = 300.0, dt: float = 0.01, steps: int = 100) -> torch.Tensor:
        ...
        Replaces the prototype that instantiated LangevinDynamics internally
        (not shared, not differentiable w.r.t. T).
        """
...
class EvolutionONEEngine:
    def __init__(self, cfg: dict = None):
        ...
        # LangevinEvolutionBridge: connect Langevin MD to population dynamics
        # Instantiated lazily when structural_langevin is available
        self.langevin_bridge: Optional[LangevinEvolutionBridge] = None
        # Bug 7: bridge to EpiForecastEngine
    ...
    def attach_epi_bridge(
        ...
        After attaching, call ``bridge.mu_to_rt(mu, rt_base)`` to couple
        mutation load into epidemic Rt, and ``bridge.rt_to_mu(rt, mu_base)``
        for the reverse direction.

        Args:
            epi_engine      : an EpiForecastEngine instance.
        ...
        # Store reference on the epi engine too for bidirectional access
    ...
    def run(self,
        ...
        # 4. RG smoothing (fully differentiable via DifferentiableRG)
        mu_tensor = torch.tensor(mu_raw, dtype=torch.float32)
        mu_smooth_tensor = self.classifier.rg(mu_tensor)   # differentiable
        mu_smooth = mu_smooth_tensor.detach().numpy()       # numpy only for downstream pandas/scipy
