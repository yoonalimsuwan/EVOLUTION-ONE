...
class InterfaceDetector(InterfaceDetectorBase):
    ...
    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        # Bug 4a: softplus floor instead of clamp (gradient everywhere)
        w_sum   = F.softplus(w.sum(dim=-1), beta=100.0) + 1e-8  # (N,)
        ...
        # Bug 4b: F.softplus replaces clamp(min=0) — smooth differentiable
        var_d   = F.softplus(mean_d2 - mean_d ** 2, beta=50.0)   # (N,)
        std_d   = torch.sqrt(var_d + 1e-8)                        # (N,)

        # Normalise std to [0,1] via sigmoid (interface score)
        return torch.sigmoid(self.sharpness * (std_d - mean_d * 0.3))  # (N,)
