# =============================================================================
# LANGEVIN ADVANCED WITH STRUCTURAL CALCULUS  (v5)
# =============================================================================
...
#
# AI Co-Developers:
#   - Claude   (Anthropic)  — cross-cluster integration, LangevinBridgeMixin,
#                             public __all__ API, CahnHilliardEvoBridge
#                             compatibility verification, import-chain audit,
#                             v5 merge & docstring restoration
#   - GPT      (OpenAI)     — literature cross-check, BAOAB numerical review
#   - Gemini   (Google)     — initial operator scaffolding
#   - DeepSeek              — alternative stencil verification
#
# Changes v4 → v5  (Documentation Restoration + Complete Integration)
#   Merge 1  — Restored full docstrings and inline comments from v3 into all
#              classes and methods (InterfaceDetector, CSOCThermostat,
#              StructuralItoNoise, AdvancedStructuralLangevin)
#   Merge 2  — Restored detailed Args/Returns blocks for every public method
#   Merge 3  — Restored v3 baoa_step() usage pattern (outer simulation loop
#              example) alongside v4's LangevinEvolutionBridge contract docs
#   Merge 4  — Restored "D^S u = ∇u + [u] δ_Γ" comment in _structural_force
#   Merge 5  — Restored full section banners (device/dtype, B/A/O steps)
#   Merge 6  — Retained all v4 additions intact:
#              __all__, from __future__ import annotations, logging,
#              LangevinEvolutionBridge, LangevinBridgeMixin,
#              CahnHilliardEvoBridge, EVOLUTION_VERSION,
#              Callable type hint for force_fn, LangevinBridgeMixin self-test
#
# Changes v3 → v4  (Cross-cluster Integration)
#   Fix 1  — Import from one_core_evolution (adds CahnHilliardEvoBridge,
#             LangevinBridgeMixin, BVFieldTheory to the shared namespace)
#   Fix 2  — Added __all__ so importers get a clean public API
#   Fix 3  — reset() made fully public and documented
#   Fix 4  — full_step() return type annotated; force_fn contract documented
#
# Changes v2 → v3  (Bug 4 — four clamp() fixes)
#   Bug 4a: w_sum.clamp(min=1e-8)      → softplus floor
#   Bug 4b: var_d.clamp(min=0.0)       → F.softplus (smooth rectifier)
#   Bug 4c: adaptive_T clamp(lo, hi)   → softplus floor + soft ceiling
#   Bug 4d: (1-c1²).clamp(min=0.0)    → F.softplus
#
...
# - LangevinBridgeMixin: attach to EvolutionONEEngine / EpiForecastEngine.
...
from one_core_evolution import (
    ...
    LangevinBridgeMixin,         # attach_langevin_bridge() mixin (Fix 1)
    EpiEvolutionBridge,          # Bug 7: cross-engine bridge
    CahnHilliardEvoBridge,       # Fix 1: CH3D ↔ EVOLUTION ONE bridge
    EVOLUTION_VERSION,
)
...
logger = logging.getLogger("StructuralLangevin")

__all__ = [
    # Re-exports from one_core_evolution
...
class InterfaceDetector(InterfaceDetectorBase):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            mode       : "auto" (default), "dense", or "chunked" — see
                         structural_interface_utils.py.
    ...
    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        # (O(N^2) memory; crashes for realistic EVOLUTION ONE population
        # sizes). This call reproduces the same softplus-floor formula
        # (Bug 4a/4b fix, use_softplus=True) exactly for small N and
        # switches to a memory-safe, still-exact chunked computation once
        # N is large — see structural_interface_utils.py for the
        # derivation and self-test.
        return soft_interface_score(
            coords, self.r_cut, self.sharpness,
            use_softplus=True, softplus_beta_sum=100.0, softplus_beta_var=50.0,
            mode=self.mode, chunk_size=self.chunk_size,
        )


# SemanticStateContraction imported from one_core_evolution

# ---------------------------------------------------------------------------
# Module 2 — CSOC Adaptive Thermostat
# ---------------------------------------------------------------------------

class CSOCThermostat(CSOCBase):
    """
    ...
    All operations are fully differentiable (no hard clamps).
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            boost_factor=temp_boost_factor,
        )
    ...
    def forward(
        ...
        dev   = self._normalised_deviation(sigma)  # from CSOCBase
        ...
        # Bug 4c: differentiable soft bounds (replaces hard clamp)
        T_lo       = self.base_temp * 0.5
        T_hi       = self.base_temp * self.temp_boost_factor
        # softplus floor
        adaptive_T = T_lo + F.softplus(adaptive_T - T_lo, beta=50.0)
        # soft ceiling via mirror softplus
        adaptive_T = T_hi - F.softplus(T_hi - adaptive_T, beta=50.0)
...
class StructuralItoNoise(StructuralItoBase):
    """
    ...
    def __init__(self, interface_amplification: float = 2.0):
        ...
        """
        super().__init__(interface_amplification=interface_amplification)
        # self.amp set by StructuralItoBase
    ...
    def get_g_matrix(self, interface_mask: torch.Tensor) -> torch.Tensor:
        ...
        """
        return self.get_g_field(interface_mask)  # from StructuralItoBase
    ...
    def compute_ito_correction(
        ...
        **kwargs,
        ...
        Args:
            field              : (N, 3) positions — must NOT already
        ...
        with torch.enable_grad():
            ...
            grad_G = torch.autograd.grad(
                G.sum(), x, create_graph=False, retain_graph=False
            )[0]                                 # (N, 3) or None
...
class AdvancedStructuralLangevin(nn.Module):
    ...
    This is the canonical Langevin integrator for the EVOLUTION ONE cluster.
    Attach to EvolutionONEEngine or EpiForecastEngine via::

        bridge = engine.attach_langevin_bridge(integrator, sigma_scale=1.5)

    which uses the LangevinBridgeMixin from one_core_evolution.

    The ``full_step()`` method implements the public API expected by
    LangevinEvolutionBridge.micro_step()::

        new_coords, new_velocities, T_scalar, sigma_scalar = \\
            integrator.full_step(coords, velocities, force_fn, jumps)

    force_fn contract::

        (energy: Tensor, force_bulk: (N, 3) Tensor) = force_fn(coords)

    Returns (scalar energy, negative energy gradient).

    For lower-level control (outer simulation loop)::
        ...
        for step in range(num_steps):
            ...
            # BAOA step — returns mid-step quantities
    ...
    def __init__(
        ...
        if mass <= 0: raise ValueError(f"mass must be positive, got {mass}")
    ...
    def reset(self) -> None:
        ...
        Call between independent trajectories / cohorts / simulations.
        """
    ...
    def _check_shape(
        ...
        if jumps.shape != force_bulk.shape:
            raise ValueError(
                f"jumps {tuple(jumps.shape)} must match "
                f"force_bulk {tuple(force_bulk.shape)}"
            )
    ...
    def baoa_step(
        ...
        for name, t in [("velocities", velocities),
            ...
            if t.device != device:
                raise RuntimeError(f"{name} on {t.device}, expected {device}")
        ...
        # Bug 4d: F.softplus replaces clamp(min=0) for smooth sqrt argument
        c2 = torch.sqrt(F.softplus(1.0 - c1 ** 2, beta=100.0) + 1e-8)
    ...
    # ------------------------------------------------------------------
    # full_step() — public API for LangevinEvolutionBridge
    # ------------------------------------------------------------------
    ...
    def full_step(
        ...
        force callable.  Implements the micro_step() API expected by
        LangevinEvolutionBridge.

        Args:
            coords      : (N, 3) current positions.
            velocities  : (N, 3) current velocities.
            force_fn    : callable(coords) → (energy: Tensor, force: (N, 3) Tensor).
        ...
        Returns:
            new_coords     : (N, 3)
            new_velocities : (N, 3)
            T_scalar       : float  adaptive temperature (K)
            sigma_scalar   : float  SSC stress (Å)
        """
        N      = coords.shape[0]
        device = coords.device
        dtype  = coords.dtype
...
# =============================================================================
# Quick self-test  (python structural_langevin_evo_v5.py)
# =============================================================================

if __name__ == "__main__":
    import sys

    print("Running AdvancedStructuralLangevin v5 self-test ...")
    ...
    # Test LangevinBridgeMixin (Fix 1 — attach_langevin_bridge)
    class DummyEngine(LangevinBridgeMixin):
        def __init__(self):
            self.langevin_bridge = None

    engine = DummyEngine()
    bridge = engine.attach_langevin_bridge(integrator, sigma_scale=1.5)
    assert bridge is engine.langevin_bridge
    print("LangevinBridgeMixin.attach_langevin_bridge() OK")
    sys.exit(0)
