...
#                             EVOLUTION ONE cross-cluster integration (v3):
#                             one_core_evolution import chain (Fix 5),
#                             CahnHilliardEvoBridge re-export & __all__,
#                             standalone SSC/CSOCBase now prefers
#                             one_core_evolution over local fallback
...
# ---------------------------------------------------------------------------
# EVOLUTION ONE cross-cluster integration (v3 — Fix 5)
# ---------------------------------------------------------------------------
# Import CahnHilliardEvoBridge from one_core_evolution so that this file
# can be used directly from the EVOLUTION ONE cluster without any code changes:
#
#   from structural_cahn_hilliard_3d import CahnHilliardEvoBridge
#
# or via engine.attach_cahn_bridge(ch_solver) which uses this bridge class.
#
# Priority:
#   1. one_core_evolution (canonical, EVOLUTION ONE cluster)
#   2. None / warning   (standalone mode — only DNS features available)
# ---------------------------------------------------------------------------
try:
    from one_core_evolution import (
        CahnHilliardEvoBridge,           # Fix 5: CH3D ↔ EVOLUTION ONE
        EpiEvolutionBridge,              # bidirectional μ ↔ Rt coupling
        EVOLUTION_VERSION,
        # If SSC/CSOCBase not yet satisfied (standalone mode), get them here
        SemanticStateContraction as _EVO_SSC,
        CSOCBase as _EVO_CSOCBase,
        InterfaceDetectorBase as _EVO_InterfaceDetectorBase,
    )
    _HAS_ONE_CORE_EVOLUTION = True
    logger.info(
        "structural_cahn_hilliard_3d: one_core_evolution v%s loaded "
        "(EVOLUTION ONE cross-cluster mode).",
        EVOLUTION_VERSION,
    )
    # If running in standalone mode without one_core or one_core_fold,
    # satisfy SSC/CSOCBase from one_core_evolution (canonical, better than local fallback)
    if not _HAS_ONE_CORE and not _HAS_ONE_CORE_FOLD:
        SemanticStateContraction  = _EVO_SSC               # type: ignore[assignment]
        CSOCBase                  = _EVO_CSOCBase           # type: ignore[assignment]
        InterfaceDetectorBase     = _EVO_InterfaceDetectorBase  # type: ignore[assignment]
        logger.info(
            "structural_cahn_hilliard_3d: SSC/CSOCBase/InterfaceDetectorBase "
            "resolved from one_core_evolution (Fix 5 standalone override)."
        )
except ImportError:
    _HAS_ONE_CORE_EVOLUTION  = False
    CahnHilliardEvoBridge    = None  # type: ignore[assignment]
    EpiEvolutionBridge       = None  # type: ignore[assignment]
    EVOLUTION_VERSION        = "unavailable"
    logger.debug(
        "structural_cahn_hilliard_3d: one_core_evolution not found — "
        "CahnHilliardEvoBridge unavailable. "
        "Install one_core_evolution.py for EVOLUTION ONE cross-cluster support."
    )

# ---------------------------------------------------------------------------
# Public API (for importers in the EVOLUTION ONE cluster and DNS cluster)
# ---------------------------------------------------------------------------
__all__ = [
    # Config
    "CahnHilliardConfig",
    # Core solver and subclasses
    "StructuralCahnHilliard3D",
    "ThinFilmStructuralCahnHilliard3D",
    "PhaseFieldCrystal3D",
    # Bridges
    "CahnHilliardDNSBridge",          # DNS cluster bridge (one_core / fallback)
    "CahnHilliardEvoBridge",          # EVOLUTION ONE bridge (Fix 5)
    "EpiEvolutionBridge",             # Epi ↔ Evo bridge (re-exported)
    # Utilities
    "structural_biharmonic_n",
    "make_sigma_field",
    # Shared bases (re-exported for convenience)
    "SemanticStateContraction",
    "CSOCBase",
    "InterfaceDetectorBase",
    # Version strings
